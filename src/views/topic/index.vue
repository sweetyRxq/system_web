//GENERATE_START
<!-- The Topic Business Entity manangement Component -->
<template>
  	<div class="app-container">
		...
		<TopicEditor ref="editor" @onOK="saveEntity"></TopicEditor>
  	</div>
...
<script>
	...
	import TopicEditor from '@/views/topic/editor'
  	export default {
		// 注册依赖组件
    	...
    	data() {
				...
				// 定义表格字段
        		header: [
    				{title: this.$t('topic.name'), field: 'name', class: ''},   
    				{title: this.$t('topic.uuid'), field: 'uuid', class: ''},   
    				{title: this.$t('topic.topicName'), field: 'topicName', class: ''}   
        		],
				// 翻页数据格式
				pagination : {
					pageSize: 10,
    	...
    	methods: {
			doSearch() {// 执行查询
				getJson('api/blockchain/topic/paginate', this.conditions, response => {
			...
			saveEntity(){// 保存和修改功能
				...
				if(mode == 'NEW'){
					postJson('api/blockchain/topic', entity, response =>{
						this.$refs.editor.close()
						this.doSearch()
					})
				}else{
					putJson('api/blockchain/topic', entity, response =>{
						this.$refs.editor.close()
						this.doSearch()
					})
				}
			...
			deleteEntity(item){// 删除功能
	            this.$confirm(this.$t('cfgentity.delete_confirm', [item.name]), this.$t('confirm'), {
	              	...
	              	deleteJson('api/blockchain/topic/' + item.uuid, {}, response => {
