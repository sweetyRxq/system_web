...
<template>
	<el-dialog :title="title" :visible.sync="visible">
		<el-form v-if="visible == true" ref="saveForm" :model="entity" status-icon label-width="120px">
			<el-form-item :label="$t('topic.name')">
			   <el-input v-if="!''" v-model.trim="entity.name"></el-input>
			</el-form-item>
			<el-form-item :label="$t('topic.uuid')">
			   <el-input v-if="!''" v-model.trim="entity.uuid"></el-input>
			</el-form-item>
			<el-form-item :label="$t('topic.topicName')">
			   <el-input v-if="!''" v-model.trim="entity.topicName"></el-input>
			</el-form-item>
		</el-form>
	...
	export default {
		...
		methods:{
			open(entity){
				...
				if(this.entity.uuid){
					this.mode = 'EDIT'
					this.title = this.$t('topic.edit')
				}else{
					this.mode = 'NEW'
					this.title = this.$t('topic.addnew')
				}
			},
			getMode(){
				return this.mode
